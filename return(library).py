...
def reading():
    ...
    val=(ee,ee1)
    cc1=mydb.cursor()
    cc1.execute("select * from issuereturn where BOOKID='%s'AND BORRID='%s'AND CATEGORY='STUDENT' AND STAT='YES'"%val);
    d=cc1.fetchall()
    for i in d:
        print(i)
    ...
    if(da>0):
        ...
        cc2.execute("update issuereturn set actualReturnDate='%s', fine=%s where BOOKID=%s AND BORRID=%s AND CATEGORY='STUDENT' AND STAT='YES'"%val1);
    else:
        cc2=mydb.cursor()
        val1=(act1,0,ee,ee1)
        cc2.execute("update issuereturn set actualReturnDate='%s', fine=%s where BOOKID=%s AND BORRID=%s AND CATEGORY='STUDENT' AND STAT='YES'"%val1);
